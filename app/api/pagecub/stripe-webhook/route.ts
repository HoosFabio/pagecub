...
export async function POST(req: Request) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (order.status !== "pending_payment") {
      console.log("[pagecub/stripe-webhook] already processed, skipping:", orderId);
    ...
    // Create sf_run directly — no tool_runs needed (pagecub uses Stripe, not credits)
    const { data: sfRun, error: sfRunErr } = await admin.from("sf_runs").insert({
      status:        "foundation_pending",
      input_payload: order.input_payload,
      // tool_run_id intentionally null — no credit billing for pagecub
    }).select("id").single();

    if (sfRunErr || !sfRun) {
      console.error("[pagecub/stripe-webhook] sf_run insert failed:", sfRunErr);
    ...
    // Dispatch Foundation Agent to MindStudio
    ...
    const token     = statusToken ?? String(order.status_token);
        ...
        body: JSON.stringify({
          ...
          html: `
            <div style="font-family:Georgia,serif;max-width:540px;margin:0 auto;padding:40px 24px;color:#243044">
              ...
              <p style="color:#6b7280;font-size:15px;margin-bottom:28px;line-height:1.7">Your payment was confirmed and the story is being built right now. It usually takes about 15–20 minutes. You can watch the progress below.</p>
              ...
              <p style="margin-top:32px;font-size:13px;color:#9ca3af;line-height:1.7">
                You'll receive another email as soon as your PDF is ready to download. Keep this email — it's your permanent access link to the book.<br><br>
                Questions? Email us at <a href="mailto:lumen@inksynth.org" style="color:#9ca3af">lumen@inksynth.org</a>
              </p>
            </div>
          `,
        }),
      });
      console.log(`[pagecub/stripe-webhook] confirmation email sent to ${order.email}`);
    } catch (err) {
